...
const recordStartButton = $('record-start');
const recordCompileButton = $('record-compile');
...
async function runDemo() {
    ...
    try {
        const trace = await jsonFetch('/api/demo/trace');
...
async function pollRun(runId) {
    for (let i = 0; i < 120; i += 1) {
        const record = await jsonFetch(`/api/runs/${runId}`);
        renderRun(record);
...
function renderPacket(packet, approval) {
...
function recordAction(action, element, value = null) {
    if (!recording)
        return;
    recordedEvents.push({
...
async function approve() {
    if (!currentRunId || !currentPacket)
        return;
    approveButton.disabled = true;
    const packet = await jsonFetch(`/api/runs/${currentRunId}/approve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ actor: 'demo-reviewer', reason: 'Shadow variants passed; export the generated automation only.' }),
    });
    currentPacket = packet;
    const record = await jsonFetch(`/api/runs/${currentRunId}`);
    renderRun(record);
}
